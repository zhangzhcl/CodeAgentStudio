...
app.whenReady().then(async () => { createApplicationMenu(); const registry = new ProviderRegistry(createCliProviders()); registerProviderIpc(registry); try { database = openDatabase(join(app.getPath('userData'), 'codeagent-studio.db')); const workspace = new WorkspaceService(new ProjectRepository(database)); registerWorkspaceIpc(workspace); workspaceIpcRegistered = true; const sessions = new SessionService(new SessionRepository(database)); registerSessionIpc(sessions); sessionIpcRegistered = true; registerAgentIpc(registry, sessions); agentIpcRegistered = true; await discoverNativeSessions(sessions, workspace); } catch (error) { console.error('[storage] SQLite unavailable, using in-memory fallback', error); const sessions = new SessionService(); if (!workspaceIpcRegistered) { registerWorkspaceIpc(new WorkspaceService()); workspaceIpcRegistered = true; } if (!sessionIpcRegistered) { registerSessionIpc(sessions); sessionIpcRegistered = true; } if (!agentIpcRegistered) { registerAgentIpc(registry, sessions); agentIpcRegistered = true; } } createWindow(); screen.on('display-metrics-changed', () => { for (const window of BrowserWindow.getAllWindows()) { const [width, height] = window.getContentSize(); const { width: screenW, height: screenH } = screen.getDisplayMatching(window.getBounds()).workAreaSize; if (width > screenW || height > screenH) window.setContentSize(Math.min(width, screenW), Math.min(height, screenH)); } }); app.on('activate', () => { if (BrowserWindow.getAllWindows().length === 0) createWindow(); }); });
