...
function inferredProjectName(path: string, provider: ProviderId): string { const folder = basename(dirname(path)); if (provider !== 'claude') return folder; const encoded = folder.replace(/^([A-Za-z])--/, '$1:/'); const decoded = encoded.replace(/--/g, '\u0000').replace(/-/g, '\\').replace(/\u0000/g, '-'); return basename(decoded) || folder; }
