...
export async function discoverNativeSessions(service: SessionService, workspace?: WorkspaceService): Promise<number> {
  ...
  for (const [provider, root] of roots) for (const path of await files(root, '.jsonl')) { const messages = await parseJsonl(path); if (!messages.length) continue; const nativeId = basename(path, '.jsonl'); const id = stableId(provider, nativeId); const cwd = await nativeCwd(path); const project = cwd && workspace ? await workspace.ensureProject(cwd).catch(() => undefined) : undefined; const existing = service.list().find((session) => session.id === id); if (!existing) { service.create({ id, provider, scope: 'project', projectId: project?.id, projectRoot: cwd, projectName: project?.name ?? (cwd ? basename(cwd) : basename(dirname(path))), nativeId, nativeSessionFile: path }); for (const message of messages) service.importMessage({ id: `${id}:native:${message.sequence}`, sessionId: id, role: message.role, content: message.content, sequence: message.sequence, createdAt: Date.now() + message.sequence }); imported++; } else if (cwd && (!existing.projectRoot || !existing.projectName || !existing.projectId)) service.updateProject(id, { projectId: project?.id, projectRoot: cwd, projectName: project?.name ?? basename(cwd) }); }
