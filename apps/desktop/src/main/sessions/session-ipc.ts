...
export const CreateSessionSchema = z.object({ provider: ProviderIdSchema, scope: SessionScopeSchema, projectId: z.string().min(1).optional() }).strict().superRefine((input, context) => {
...
export function registerSessionIpc(service: SessionService, workspace?: WorkspaceService) {
  ...
  ipcMain.handle('session:create', (_event, input: unknown) => { const parsed = CreateSessionSchema.parse(input); if (parsed.scope === 'project' && !workspace?.listProjects().some((project) => project.id === parsed.projectId)) throw new Error('Unknown registered project'); return service.create(parsed); });
