...
export class ProviderRuntime {
  ...
  async start(sessionId: string, providerId: AgentProvider['id'], input: Parameters<AgentProvider['createSession']>[0]) {
    ...
    try { await provider.createSession(input); this.active.set(sessionId, { sessionId, provider: providerId, state: 'running' }); } catch (error) { this.active.set(sessionId, { sessionId, provider: providerId, state: 'error' }); throw error; }
  }
