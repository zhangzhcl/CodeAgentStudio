...
export class CliProvider implements AgentProvider {
  ...
  async prompt(sessionId: string, text: string, model?: string, options?: PromptOptions) {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const rawArgs = [...(this.config.commandArgs ?? []), ...this.config.promptArgs(model), ...(useStdin ? [] : [text])]; const args = this.config.shell ? rawArgs.map((arg) => quoteShellArg(arg)) : rawArgs; const child = spawn(this.config.command, args, { shell: this.config.shell, windowsHide: true, cwd: this.sessionCwds.get(sessionId), env: withUserBinaryPaths(process.env), stdio: [useStdin ? 'pipe' : 'ignore', 'pipe', 'pipe'] }); if (useStdin && child.stdin) { // 子进程可能先退出，忽略 EPIPE 防止未处理异常
      ...
      child.stdout?.on('data', consume); child.stderr?.on('data', (data) => { const line = data.toString(); if (/no api key|not logged in|authentication required|rate limit|timed out|error/i.test(line)) { const event = parseCliEvent(line, this.id, sessionId, sequence++, runMessageId); if (event?.type === 'error') this.listeners.forEach((listener) => listener(event)); } }); child.once('error', reject); child.once('close', (code) => { if (pending.trim()) { const event = parseCliEvent(pending, this.id, sessionId, sequence++, runMessageId); if (event) this.listeners.forEach((listener) => listener(event)); } this.processes.delete(sessionId); const wasAborted = this.aborted.delete(sessionId); if (!wasAborted && typeof code === 'number' && code !== 0) { const failure = parseCliEvent(JSON.stringify({ type: 'error', code: 'process_exit', message: `Agent exited with code ${code}` }), this.id, sessionId, sequence++, runMessageId); if (failure) this.listeners.forEach((listener) => listener(failure)); } else if (!wasAborted) { const done = parseCliEvent(JSON.stringify({ type: 'done' }), this.id, sessionId, sequence++, runMessageId); if (done) this.listeners.forEach((listener) => listener(done)); } resolve(); });
    });
  }
