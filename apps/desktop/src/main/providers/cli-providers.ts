...
export class CliProvider implements AgentProvider {
  ...
  async prompt(sessionId: string, text: string) {
    await new Promise<void>((resolve, reject) => {
      ...
      child.stdout?.on('data', consume); child.stderr?.on('data', (data) => { const line = data.toString(); if (/no api key|not logged in|authentication required|rate limit|timed out|error/i.test(line)) { const event = parseCliEvent(line, this.id, sessionId, sequence++); if (event?.type === 'error') this.listeners.forEach((listener) => listener(event)); } }); child.once('error', reject); child.once('close', (code) => { if (pending.trim()) { const event = parseCliEvent(pending, this.id, sessionId, sequence++); if (event) this.listeners.forEach((listener) => listener(event)); } this.processes.delete(sessionId); if (typeof code === 'number' && code !== 0) { const failure = parseCliEvent(JSON.stringify({ type: 'error', code: 'process_exit', message: `Agent exited with code ${code}` }), this.id, sessionId, sequence++); if (failure) this.listeners.forEach((listener) => listener(failure)); } else { const done = parseCliEvent(JSON.stringify({ type: 'done' }), this.id, sessionId, sequence++); if (done) this.listeners.forEach((listener) => listener(done)); } resolve(); });
    });
  }
