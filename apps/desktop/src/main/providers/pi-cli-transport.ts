...
export class PiCliTransport implements PiTransport {
  private readonly running = new Map<string, ReturnType<typeof spawn>>(); private readonly listeners = new Set<(event: AgentEvent) => void>(); private readonly resumed = new Set<string>();
  ...
  async resumeSession(nativeId: string, _sessionFile: string) { this.resumed.add(nativeId); }
  async prompt(nativeId: string, text: string) { await new Promise<void>((resolve, reject) => { const provider = process.env.CODEAGENT_PI_PROVIDER ?? 'sensenova'; const model = process.env.CODEAGENT_PI_MODEL ?? 'sensenova-6.8-flash-lite'; const resume = this.resumed.delete(nativeId); const child = spawn(this.command, buildPiPromptArgs(provider, model, nativeId, text, resume), { shell: process.platform === 'win32', windowsHide: true, env: withUserBinaryPaths(process.env), stdio: ['ignore', 'pipe', 'pipe'] }); this.running.set(nativeId, child); let sequence = 0; let pending = ''; const consume = (data: Buffer) => { pending += data.toString(); const lines = pending.split(/\r?\n/); pending = lines.pop() ?? ''; lines.forEach((line) => { const event = parseCliEvent(line, 'pi', nativeId, sequence++); if (event) this.listeners.forEach((listener) => listener(event)); }); }; child.stdout?.on('data', consume); child.stderr?.on('data', consume); child.once('error', reject); child.once('close', () => { if (pending.trim()) { const event = parseCliEvent(pending, 'pi', nativeId, sequence++); if (event) this.listeners.forEach((listener) => listener(event)); } this.running.delete(nativeId); resolve(); }); }); }
...
export function buildPiPromptArgs(provider: string, model: string, nativeId: string, text: string, resume: boolean): string[] {
  return ['--print', '--mode', 'json', '--provider', provider, '--model', model, resume ? '--resume' : '--session', nativeId, text];
}
