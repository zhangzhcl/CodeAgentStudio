...
export class PiCliTransport implements PiTransport {
  ...
  detect(): Promise<ProviderStatus> { return new Promise((resolve) => { const child = spawn(this.command, ['--version'], { shell: process.platform === 'win32', windowsHide: true, env: withUserBinaryPaths(process.env) }); let output = ''; child.stdout?.on('data', (data) => { output += data.toString(); }); child.once('error', () => resolve({ provider: 'pi', command: this.command, installed: false, authenticated: false, errorCode: 'not_installed' })); child.once('close', (code) => resolve({ provider: 'pi', command: this.command, installed: code === 0, authenticated: false, version: output.trim() || undefined, errorCode: code === 0 ? undefined : 'unknown' })); }); }
}
