...
describe('database', () => {
  it('creates all tables and migrates an empty database to the current schema', async () => {
    ...
    expect((db.prepare('SELECT MAX(version) as version FROM schema_version').get() as { version: number }).version).toBe(7);
    expect((db.prepare("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").all() as Array<{ name: string }>).map((item) => item.name)).toEqual(['messages', 'projects', 'schema_version', 'session_native_runs', 'sessions']);
  ...
  it('upgrades a legacy v1 database through all migrations and preserves projects as user projects', async () => {
    ...
    const db = openDatabase(file); expect((db.prepare('SELECT MAX(version) as version FROM schema_version').get() as { version: number }).version).toBe(7); expect((db.prepare('SELECT source FROM projects WHERE id=?').get('p') as { source: string }).source).toBe('user'); expect((db.prepare('PRAGMA table_info(sessions)').all() as Array<{ name: string }>).some((column) => column.name === 'title')).toBe(true); db.close(); await rm(dir, { recursive: true, force: true });
  });
