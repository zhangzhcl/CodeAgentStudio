...
function isCorruptDatabase(error: unknown) { const value = error as { code?: string; message?: string }; return value.code === 'SQLITE_CORRUPT' || value.code === 'SQLITE_NOTADB' || /database disk image is malformed|file is not a database/i.test(value.message ?? ''); }
