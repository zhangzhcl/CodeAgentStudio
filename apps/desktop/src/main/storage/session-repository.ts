...
export class SessionRepository {
  ...
  listMessages(sessionId: string) { return (this.db.prepare('SELECT id,session_id as sessionId,role,content,sequence,created_at as createdAt FROM messages WHERE session_id=? ORDER BY sequence').all(sessionId) as Array<Omit<MessageRecord, 'content'> & { content: string }>).map((message) => ({ ...message, content: JSON.parse(message.content) })); }
}
