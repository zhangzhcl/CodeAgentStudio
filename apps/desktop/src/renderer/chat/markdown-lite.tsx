...
export function MarkdownLite({ content }: { content: string }) {
  ...
  for (let index = 0; index < blocks.length; index += 1) {
    if (index % 3 === 0) { const text = blocks[index] ?? ''; text.split(/\n{2,}/).filter(Boolean).forEach((paragraph, partIndex) => { const lines = paragraph.split('\n'); if (lines.every((line) => /^[-*] \[[ x~>]\] /.test(line))) nodes.push(<div className="task-list-block" key={`tasks-${index}-${partIndex}`}><div className="task-list-head">执行计划 <span>{lines.filter((line) => /^[-*] \[x\] /i.test(line)).length}/{lines.length}</span></div>{lines.map((line, taskIndex) => { const marker = line.match(/^[-*] \[([ x~>])\] /)?.[1] ?? ' '; const status = marker.toLowerCase() === 'x' ? 'done' : marker === '>' || marker === '~' ? 'running' : 'pending'; return <div className={`task-list-item task-${status}`} key={`${line}-${taskIndex}`}><span className="task-check" aria-hidden="true">{status === 'done' ? '✓' : status === 'running' ? '·' : ''}</span><span><InlineMarkdown text={line.replace(/^[-*] \[[ x~>]\] /, '')} /></span></div>; })}</div>); else if (lines.every((line) => /^[-*] /.test(line))) nodes.push(<ul className="md-list" key={`list-${index}-${partIndex}`}>{lines.map((line) => <li key={line}><InlineMarkdown text={line.slice(2)} /></li>)}</ul>); else if (lines.every((line) => /^\d+\. /.test(line))) nodes.push(<ol className="md-list" key={`ordered-${index}-${partIndex}`}>{lines.map((line) => <li key={line}><InlineMarkdown text={line.replace(/^\d+\. /, '')} /></li>)}</ol>); else if (lines.every((line) => /^> ?/.test(line))) nodes.push(<blockquote className="md-quote" key={`quote-${index}-${partIndex}`}><InlineMarkdown text={lines.map((line) => line.replace(/^> ?/, '')).join('\n')} /></blockquote>); else if (/^#{1,3} /.test(lines[0] ?? '')) { const level = Math.min(3, (lines[0]!.match(/^#+/)?.[0].length ?? 1)); const Heading = level === 1 ? 'h2' : level === 2 ? 'h3' : 'h4'; nodes.push(<Heading key={`heading-${index}-${partIndex}`}><InlineMarkdown text={lines[0]!.replace(/^#+\s*/, '')} /></Heading>); if (lines.length > 1) nodes.push(<p key={`heading-text-${index}-${partIndex}`}><InlineMarkdown text={lines.slice(1).join('\n')} /></p>); } else nodes.push(<p key={`text-${index}-${partIndex}`}><InlineMarkdown text={paragraph} /></p>); }); continue; }
