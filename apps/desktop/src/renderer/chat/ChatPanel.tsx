...
export function ChatPanel({ sessionId, providerName = 'Agent', providers = ['Claude', 'Cursor', 'Codex', 'Pi', 'OpenCode'], subscribe, onPrompt }: Props) {
  ...
  return <section className="chat-panel" aria-label="Agent 对话">
    ...
    <div role="log" aria-live="polite">{messages.length === 0 ? <p>输入消息开始对话。</p> : messages.map((message) => <article key={message.id} data-role={message.role}><strong>{message.role === 'user' ? '你' : provider}</strong><p>{message.content}{message.status === 'streaming' ? '▍' : ''}</p></article>)}</div>
