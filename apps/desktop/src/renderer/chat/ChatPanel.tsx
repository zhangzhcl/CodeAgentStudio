...
export function ChatPanel({ sessionId, providerName = 'Claude', providers = ['Claude', 'Cursor', 'Codex', 'Pi', 'OpenCode'], disabledProviders = [], loadMessages, subscribe, onPrompt, onAbort }: Props) {
  ...
  useEffect(() => { let active = true; sequencer.current = new EventSequencer(); setMessages([]); if (loadMessages) void loadMessages().then((loaded) => { if (active) setMessages(loaded); }); return () => { active = false; }; }, [sessionId]);
  ...
  return <section className="chat-panel" aria-label="Agent 对话">
    ...
    <form onSubmit={(event) => { event.preventDefault(); void send(); }}><textarea aria-label="消息" value={draft} onChange={(event) => setDraft(event.target.value)} disabled={sending} /><button type="submit" disabled={!draft.trim() || sending}>发送</button>{sending && <button type="button" onClick={() => { setSending(false); void onAbort?.(sessionId); }}>停止</button>}</form>
  </section>;
}
