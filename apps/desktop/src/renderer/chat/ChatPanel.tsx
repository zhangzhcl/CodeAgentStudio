...
type Props = { sessionId: string; providerName?: string; scope?: 'personal' | 'project'; projectName?: string; providers?: string[]; disabledProviders?: string[]; loadMessages?: () => Promise<ChatMessage[]>; subscribe?: (listener: (event: AgentEvent) => void) => () => void; onPrompt?: (text: string, provider: string) => Promise<void> | void; onAbort?: (sessionId: string) => Promise<unknown> | unknown; onProviderChange?: (provider: string) => void; onTitleChange?: (title: string) => void };


export function ChatPanel({ sessionId, providerName = 'Claude', scope = 'personal', projectName, providers = ['Claude', 'Cursor', 'Codex', 'Pi', 'OpenCode'], disabledProviders = [], loadMessages, subscribe, onPrompt, onAbort, onProviderChange, onTitleChange }: Props) {
  ...
  return <section className="chat-panel" aria-label="Agent 对话">
    ...
    <div ref={transcriptRef} className="chat-transcript" role="log" aria-live="polite" onScroll={(event) => { const element = event.currentTarget; const atBottom = element.scrollHeight - element.scrollTop - element.clientHeight < 80; stickToBottom.current = atBottom; setShowScrollButton(!atBottom && element.scrollHeight > element.clientHeight + 80); }}>{messages.length === 0 ? <div className="chat-empty">{scope === 'project' && projectName && <div className="project-context-strip">项目上下文 · {projectName}</div>}<div className="empty-mark">✦</div><span className="welcome-status">SYSTEM READY</span><h2>今天想让 Agent 做什么？</h2><p>从一个快捷任务开始，或直接描述你的代码问题。</p><div className="quick-prompts"><button type="button" onClick={() => useQuickPrompt('帮我快速了解这个项目的目录结构和主要技术栈')}>了解项目</button><button type="button" onClick={() => useQuickPrompt('检查当前项目中最值得优先修复的问题，并给出修复计划')}>检查问题</button><button type="button" onClick={() => useQuickPrompt('帮我实现一个小功能，并先说明你准备修改哪些文件')}>实现功能</button><button type="button" onClick={() => useQuickPrompt('阅读当前代码，找出潜在的性能或安全风险')}>代码审查</button></div></div> : messages.map((message) => <MessageItem key={message.id} message={message} provider={provider} copied={copiedMessage === message.id} sending={sending} onCopy={(id, content) => void copyMessage(id, content)} onRegenerate={() => void regenerate()} onFeedback={() => setComposerNotice('已记录反馈')} onEditResend={(text) => void runPrompt(text)} onSuggestion={useQuickPrompt} />)}</div>
    {showScrollButton && <button type="button" className="scroll-bottom" aria-label="回到底部" title="回到底部" onClick={scrollToBottom}>↓</button>}
