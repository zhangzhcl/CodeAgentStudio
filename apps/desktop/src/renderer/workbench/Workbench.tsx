...
export function Workbench() {
  ...
  const [registeredProjects, setRegisteredProjects] = useState<Array<{ id: string; name?: string; rootPath?: string }>>([]);
  const [personalSessions, setPersonalSessions] = useState<Array<{ id: string; provider: string; title?: string; projectId?: string; projectName?: string; projectRoot?: string; nativeId?: string; updatedAt?: number }>>([]);
  const [projectSessions, setProjectSessions] = useState<Array<{ id: string; provider: string; title?: string; projectId?: string; projectName?: string; projectRoot?: string; nativeId?: string; updatedAt?: number }>>([]);
  ...
  const relativeTime = (timestamp?: number) => { if (!timestamp) return ''; const minutes = Math.max(0, Math.round((Date.now() - timestamp) / 60000)); if (minutes < 1) return '刚刚'; if (minutes < 60) return `${minutes} 分钟前`; const hours = Math.round(minutes / 60); if (hours < 24) return `${hours} 小时前`; return `${Math.round(hours / 24)} 天前`; };
  ...
  const closeTab = (tab: WorkbenchTab) => { setTabs((items) => { const index = items.findIndex((item) => isSameTab(item, tab)); const next = items.filter((item) => !isSameTab(item, tab)); if (isSameTab(activeTab, tab)) { const fallback = next[index] ?? next[index - 1] ?? { kind: 'chat' as const, sessionId: 'new-chat', scope: 'personal' as const }; setActiveTab(fallback); } return next; }); };
  const deleteActiveSession = () => {
    ...
    setProjectSessions((items) => items.filter((item) => item.id !== activeTab.sessionId));
    closeTab(activeTab);
  };
        ...
        <section aria-label="侧栏">
          {activity === 'files' ? (
            ...
            <div>
              ...
              {sessionView === 'sessions' ? <><h3 className="session-section-label">个人会话</h3><button type="button" onClick={() => newSession('personal')}>＋ 新建个人会话</button>{visiblePersonalSessions.length === 0 ? <p>暂无 {providerLabel(selectedProvider)} 个人会话</p> : visiblePersonalSessions.map(({ id, provider, projectId: sessionProjectId, nativeId, title, updatedAt }) => <button type="button" aria-current={activeTab.kind === 'chat' && activeTab.sessionId === id} key={id} onClick={() => openSession(id, 'personal', provider, sessionProjectId)}><span>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</span><small>{relativeTime(updatedAt)}</small></button>)}</> : <>{projectGroups.size === 0 ? <p>暂无 {providerLabel(selectedProvider)} 项目</p> : [...projectGroups.entries()].map(([key, group]) => <div className="session-project-group" data-expanded={group.sessions.length > 0 ? 'true' : 'false'} key={key}><div className="project-group-header"><h3 title={group.root}><span>{group.name}</span><small>{group.sessions.length}</small></h3><span className="project-caret" aria-hidden="true">⌄</span><button type="button" aria-label={`在 ${group.name} 中新建会话`} onClick={() => { const registered = registeredProjects.find((project) => project.id === key); if (registered) { setProjectId(registered.id); setProjectName(registered.name ?? registered.rootPath?.split(/[\\/]/).pop() ?? '项目'); setProjectRoot(registered.rootPath); } else if (group.sessions[0]?.projectId) setProjectId(group.sessions[0].projectId); newSession('project'); }}>＋</button></div>{group.sessions.length === 0 ? <p className="project-no-sessions">暂无会话，点击 + 新建</p> : group.sessions.map(({ id, provider, projectId: sessionProjectId, projectName, projectRoot, nativeId, title, updatedAt }) => <button type="button" aria-current={activeTab.kind === 'chat' && activeTab.sessionId === id} title={projectRoot ?? projectName ?? '未关联项目'} key={id} onClick={() => openSession(id, 'project', provider, sessionProjectId)}><span>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</span><small>{relativeTime(updatedAt)}</small><span role="button" tabIndex={0} className="session-row-delete" aria-label={`删除会话 ${sessionTitle(id, nativeId, title)}`} onClick={(event) => { event.stopPropagation(); if (!window.confirm('确定删除当前会话及其历史消息吗？')) return; const remove = (window as Window & { codeagentSessions?: { delete?: (value: string) => Promise<unknown> } }).codeagentSessions?.delete; if (remove) void remove(id); setProjectSessions((items) => items.filter((item) => item.id !== id)); setPersonalSessions((items) => items.filter((item) => item.id !== id)); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); (event.currentTarget as HTMLElement).click(); } }}>×</span></button>)}</div>)}</>}
            </div>
          )}
        </section>
