        ...
        <section aria-label="侧栏">
          {activity === 'files' ? (
            ...
            <div>
              ...
              {sessionView === 'sessions' ? <><h3 className="session-section-label">个人会话</h3><button type="button" onClick={() => newSession('personal')}>＋ 新建个人会话</button>{visiblePersonalSessions.length === 0 ? <p>暂无 {providerLabel(selectedProvider)} 个人会话</p> : visiblePersonalSessions.map(({ id, provider, projectId: sessionProjectId, nativeId, title }) => <button type="button" key={id} onClick={() => openSession(id, 'personal', provider, sessionProjectId)}>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</button>)}<h3 className="session-section-label">最近项目会话</h3>{visibleProjectSessions.slice(0, 20).map(({ id, provider, projectId: sessionProjectId, projectName, projectRoot, nativeId, title }) => <button type="button" title={projectRoot ?? projectName ?? '未关联项目'} key={id} onClick={() => openSession(id, 'project', provider, sessionProjectId)}>{projectName ? `${projectName} · ` : ''}{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</button>)}</> : <>{projectGroups.size === 0 ? <p>暂无 {providerLabel(selectedProvider)} 项目</p> : [...projectGroups.entries()].map(([key, group]) => <div className="session-project-group" key={key}><div className="project-group-header"><h3 title={group.root}>{group.name}</h3><button type="button" aria-label={`在 ${group.name} 中新建会话`} onClick={() => { if (group.sessions[0]?.projectId) setProjectId(group.sessions[0].projectId); newSession('project'); }}>＋</button></div>{group.sessions.map(({ id, provider, projectId: sessionProjectId, projectName, projectRoot, nativeId, title }) => <button type="button" title={projectRoot ?? projectName ?? '未关联项目'} key={id} onClick={() => openSession(id, 'project', provider, sessionProjectId)}>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</button>)}</div>)}</>}
            </div>
          )}
        </section>
      </aside>
