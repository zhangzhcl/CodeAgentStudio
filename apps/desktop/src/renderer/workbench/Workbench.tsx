        ...
        <section aria-label="侧栏">
          {activity === 'files' ? (
            ...
            <div>
              ...
              {sessionView === 'sessions' ? <><h3 className="session-section-label">个人会话</h3><button type="button" onClick={() => newSession('personal')}>＋ 新建个人会话</button>{visiblePersonalSessions.length === 0 ? <p>暂无 {providerLabel(selectedProvider)} 个人会话</p> : visiblePersonalSessions.map(({ id, provider, projectId: sessionProjectId, nativeId, title, updatedAt }) => <button type="button" key={id} onClick={() => openSession(id, 'personal', provider, sessionProjectId)}><span>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</span><small>{relativeTime(updatedAt)}</small></button>)}</> : <>{projectGroups.size === 0 ? <p>暂无 {providerLabel(selectedProvider)} 项目</p> : [...projectGroups.entries()].map(([key, group]) => <div className="session-project-group" key={key}><div className="project-group-header"><h3 title={group.root}>{group.name}</h3><button type="button" aria-label={`在 ${group.name} 中新建会话`} onClick={() => { if (group.sessions[0]?.projectId) setProjectId(group.sessions[0].projectId); newSession('project'); }}>＋</button></div>{group.sessions.map(({ id, provider, projectId: sessionProjectId, projectName, projectRoot, nativeId, title, updatedAt }) => <button type="button" title={projectRoot ?? projectName ?? '未关联项目'} key={id} onClick={() => openSession(id, 'project', provider, sessionProjectId)}><span>{providerLabel(provider)} · {sessionTitle(id, nativeId, title)}</span><small>{relativeTime(updatedAt)}</small><span role="button" tabIndex={0} className="session-row-delete" aria-label={`删除会话 ${sessionTitle(id, nativeId, title)}`} onClick={(event) => { event.stopPropagation(); if (!window.confirm('确定删除当前会话及其历史消息吗？')) return; const remove = (window as Window & { codeagentSessions?: { delete?: (value: string) => Promise<unknown> } }).codeagentSessions?.delete; if (remove) void remove(id); setProjectSessions((items) => items.filter((item) => item.id !== id)); setPersonalSessions((items) => items.filter((item) => item.id !== id)); }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); event.stopPropagation(); (event.currentTarget as HTMLElement).click(); } }}>×</span></button>)}</div>)}</>}
            </div>
          )}
        </section>
