...
export function FileExplorer({
  ...
  const resolvedList =
    ...
    (projectId
      ? async (path: string) => {
          ...
          return api ? api.list(projectId, path) : [];
        }
      : async () => []);
  ...
  const toggle = async (entry: WorkspaceEntry) => {
    ...
    if (expanded.has(entry.path)) {
      setExpanded(
        (current) =>
          new Set([...current].filter((path) => path !== entry.path)),
      );
      return;
    }
  ...
  const renderEntries = (items: WorkspaceEntry[], depth = 0) =>
    items
      ...
      .map((entry) => (
        <li key={entry.path}>
          ...
          {entry.isDirectory && expanded.has(entry.path) && (
            <ul>{renderEntries(children[entry.path] ?? [], depth + 1)}</ul>
          )}
        </li>
      ));

  if (!projectId && !listEntries && initialEntries.length === 0)
    return (
      <div role="tree" aria-label="文件资源管理器">
        <p className="file-empty">请选择一个项目以查看文件</p>
      </div>
    );
  return (
    <section className="file-explorer" aria-label="文件资源管理器">
      ...
      <div className="file-project-row">
        ...
        <div className="file-project-actions">
          ...
          <button type="button" aria-label="新建文件夹">
            <IconPlus size={14} />
          </button>
          <button
            type="button"
            aria-label="刷新文件树"
            onClick={() => void resolvedList("").then(setEntries)}
          >
            <IconRefresh size={14} />
          </button>
        </div>
